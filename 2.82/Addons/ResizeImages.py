...
class BR_OT_resize_image(bpy.types.Operator):
    """Render 3d Photo for facebook, both images are saved next to .blend"""
    bl_idname = "render.resize_image"
    bl_label = "Choose Render Resolution"

    width = IntProperty(name="Size", description="Width in pixels for rendered image", min=32, max=8196, default=512)
    ...
    def execute(self, context):
        ...
        output = ""
        overwrite = True
        if self.all_images == True :
            w_man.ris_all_images = 'All images'
        else:
            w_man.ris_all_images = 'Active image'
        all = w_man.ris_all_images
        cleanup = False
        width = self.width
...
class DialogPanel(bpy.types.Panel):
    bl_idname = "Choose output image resolution"
    bl_space_type = "VIEW_3D"
    bl_region_type = 'TOOLS'

    def draw(self, context):
            self.layout.operator(BR_OT_resize_image.bl_idname)

def menu_draw(self, context):
    self.layout.operator("render.resize_image", 
        text="Resize Image...")
...
def register():
    ...
    for cls in classes:
        register_class(cls)
    bpy.types.IMAGE_MT_image.append(menu_draw)
